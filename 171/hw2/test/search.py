...
class EightPuzzleSearchProblem(Problem):
    ...
    def get_index(self, state, value=0):
    ...
    def actions(self, state):
        ...
        for coord in range(2):
            if index[coord] - 1 >= 0:
                valid.append('left' if coord else 'up')
            if index[coord] + 1 <= 2:
                valid.append('right' if coord else 'down')
        return valid

    def result(self, state, action):
        self.num_states += 1
        new = list(map(list, state))
        r1, c1 = self.get_index(new)
        r2, c2 = self.dispatch[action](r1, c1)
        new[r1][c1], new[r2][c2] = new[r2][c2], new[r1][c1]
        return tuple(map(tuple, new))

    def h(self, node):
        """
        Returns Manhattan distance of 8-puzzle
        """
        state = node.state
        cost = 0
        for i in '12345678':
            r1, c1 = self.get_index(state, int(i))
            r2, c2 = self.get_index(self.goal, int(i))
...
class TSPSearchProblem(Problem):
    """
    A problem which is solved by an agent executing
    actions, rather than by just computation.
    Carried in a deterministic and a fully observable environment.
    ...
    def get_mst(self, root):
        ...
        for v in nodes:
            pqueue[v] = key[v]
        while pqueue:
            src = self.popmin(pqueue)
            for dest, cost in self.graph.get(src).items():
                if dest in pqueue and cost < key[dest]:
                    pred[dest] = src
                    key[dest] = cost
                    pqueue[dest] = cost
        return key
    ...
    def h(self, node):
        ...
        Returns Manhattan distance of 8-puzzle
        """
        neighbors = self.graph.get(node.state)
        closest = min(neighbors, key=neighbors.get)
        first = self.get_mst(node.state)[closest]
        second = self.c(node.state, closest, closest)
        third = self.get_mst(self.initial)[closest]
